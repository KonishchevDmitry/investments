...
Всю необходимую информацию можно взять из отчета:
* `Остаток денежных средств на счете на начало отчетного периода` — `Начальная сумма средств` в отчете
* `Зачислено денежных средств за отчетный период` — сумма всех положительных значений между `Начальная сумма средств` и `Остаток средств на конец периода` в отчете
* `Списано денежных средств за отчетный период` — сумма всех отрицательных значений между `Начальная сумма средств` и `Остаток средств на конец периода` в отчете
* `Остаток денежных средств на счете на конец отчетного периода` — `Остаток средств на конец периода` в отчете

Также можно воспользоваться командой `cash-flow`. Но с одной оговоркой: т. к. при [возврате налогов](#dividend-reclassifications)
в Activity Statement указываются не реальные даты зачисления средств на счет, а даты выплаты дивиденда / первичного
удержания налога, то у программы не будет всех необходимых данных, чтобы корректно рассчитать движение средств, и ей
придется гадать, когда на самом деле были зачислены/удержаны средства. Решить эту проблему можно использованием [Custom
Activity Statement](brokers.md#ib-custom-activity-statement) - в нем присутствуют все необходимые для расчета данные.